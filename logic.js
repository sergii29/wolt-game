...
const ITEMS_DB = {
    ...
    water: { name: 'Вода (0.5л)', type: 'buy', cost: 2.0, icon: 'fa-bottle-water', effect: { water: 35, energy: 5 }, desc: 'Спасает от жажды.', target: 'water' },
    bar: { name: 'Сникерс', type: 'buy', cost: 4.5, icon: 'fa-cookie-bite', effect: { energy: 20, mood: 15 }, desc: 'Баланс энергии.', target: 'energy' },
    energy_drink: { name: 'Red Bull', type: 'buy', cost: 7.0, icon: 'fa-bolt', effect: { energy: 50, mood: 5, water: -5 }, desc: 'МАКСИМУМ Энергии!', target: 'energy' },
    coffee: { name: 'Латте', type: 'buy', cost: 9.0, icon: 'fa-mug-hot', effect: { mood: 30, energy: 10 }, desc: 'Для Настроения!', target: 'mood' }
};

let state = {
    balance: 0,
    debt: 0, debtTimer: 0, debtOverdue: false,
...
function init() {
    ...
    initSliders();
    setInterval(checkDebt, 1000); // Check debt often for timer update
}
...
function checkDebt() {
    ...
    if (state.debt > 0 && !state.debtOverdue) {
        if (Date.now() > state.debtTimer) {
            ...
            updateUI();
            // Refresh bank modal if open to show "Overdue"
            if(document.getElementById('full-modal').classList.contains('open') && document.getElementById('modal-title').textContent.includes('Банк')) {
                renderBank(document.getElementById('modal-body'));
            }
        } else {
            // Update timer in modal if open
            if(document.getElementById('full-modal').classList.contains('open') && document.getElementById('modal-title').textContent.includes('Банк')) {
               renderBank(document.getElementById('modal-body')); // Live update of timer
            }
        }
    }
}
...
function updateBar(id, val) { document.getElementById(id).style.width = Math.max(0, Math.min(100, val)) + '%'; }
...
function startOrderLoop() {
    ...
    orderInterval = setInterval(() => {
        ...
        if (currentOrder.foodTemp < 50 && !currentOrder.tempWarned) {
            showToast('❄️ Еда остывает! Поторопись!', 'warn');
            currentOrder.tempWarned = true;
        }
        
        document.getElementById('temp-val').textContent = Math.floor(currentOrder.foodTemp) + '%';

        if (currentOrder.foodTemp <= 0) {
            failOrder("cold");
            return;
        }
...
function failOrder(reason) {
    ...
    showToast(`${msg} Штраф -5.00 PLN`, 'warn');
    state.balance -= 5.00;
    currentOrder = null;
    saveGame(); updateUI();
    startSearching();
...
function pedal() {
    ...
    if (state.needs.energy <= 5 || state.needs.water <= 5) {
        showToast('Сил нет! Открой рюкзак!', 'warn');
        openQuickInv(true);
        return;
    }

    // WEAR & CONSUMPTION (Increased)
    state.needs.energy -= 0.15; 
    state.needs.water -= 0.10;
    state.needs.mood -= 0.05;
    state.items.bike -= 0.1; // Increased wear on bike
    state.items.phone -= 0.05; // Battery wear
...
function triggerRandomEvent() {
    ...
    const events = [
        { text: "🚧 ПРОБКА!", time: 3000 },
        { text: "🚦 СВЕТОФОР", time: 2000 },
        { text: "🚕 ОБЪЕЗД", time: 4000 }
    ];
...
function acceptOrder() {
    ...
    startOrderLoop(); // Start physics
}

function missOrder() {
    state.isSearching = true; 
    currentOrder = null;
    state.balance -= 0.10; // Fixed penalty
    showToast('Заказ упущен! Штраф -0.10 PLN', 'warn');
    saveGame(); updateUI();
    startSearching();
}
...
function nextStage() {
    ...
    if (currentOrder.stage === 1) { // Waiting
        const btn = document.getElementById('pedal-btn');
        btn.disabled = true; btn.textContent = 'ЖДЕМ ЗАКАЗ...';
        document.getElementById('status-label').textContent = 'Ресторан готовит...';
        setTimeout(() => {
            btn.disabled = false; btn.textContent = 'ЖМИ ГАЗ (PEDAL)';
            document.getElementById('status-label').textContent = 'Везем клиенту';
...
function completeOrder() {
    ...
    const inflationMult = 1 + (totalRepairs * 0.05);
    
    let pay = (12 + (Math.random() * 6)) * inflationMult;
    let tip = 0;

    // --- TIPPING LOGIC ---
    if (currentOrder.foodTemp > 60) {
        if (Math.random() > 0.5) tip += (2 + Math.random() * 3) * inflationMult;
    }
    if (state.needs.mood > 80 && currentOrder.foodTemp > 40) {
        if (Math.random() > 0.3) tip += 3 * inflationMult;
    }

    if (tip > 0) {
        pay += tip;
        let msg = (state.needs.mood > 80 && currentOrder.foodTemp > 70) 
            ? `🔥 Быстро и весело! +${tip.toFixed(2)} чай!` 
            : `👍 Клиент доволен! +${tip.toFixed(2)} чай`;
        showToast(msg, 'success');
    } else if (currentOrder.foodTemp < 50) {
        showToast('Еда остыла... Без чаевых.', 'warn');
    }

    // Apply wear to Bag and Gear on completion
    state.items.bag -= 1.5; 
    state.items.gear -= 0.8;
...
function renderShop(c) {
    ...
    const inflation = 1 + (levelSum * 0.1); 
    ...
    ['bike', 'bag', 'phone', 'gear'].forEach(k => {
        ...
        <div class="shop-card">
            ...
            <button class="shop-btn" 
                ${isBroken ? '' : 'disabled'} 
                onclick="buyRepair('${k}', ${price}, this)">
                ${btnText}
            </button>
        </div>`;
    });
...
function renderDeflation(c) {
    const levelSum = Object.values(state.repairs).reduce((a,b)=>a+b,0);
    const inflation = 1 + (levelSum * 0.1);
    const cost1 = 2700 * inflation;
    const cost2 = 5000 * inflation;
    ...
    c.innerHTML = `
        ...
        <div class="shop-card">
            <div>
                <div style="font-weight:bold">Откат (-1 уровень)</div>
                <div class="shop-desc">Снижает цены на шаг назад</div>
            </div>
            <button class="shop-btn govt-btn" onclick="buyDeflation(1, ${cost1})">-${cost1.toFixed(0)}</button>
        </div>
        <div class="shop-card">
            <div>
                <div style="font-weight:bold">Сильный откат (-2 ур.)</div>
                <div class="shop-desc">Серьезное снижение цен</div>
            </div>
...
function resetGame() {
    if(confirm('Сбросить прогресс? Вы начнете новую игру с долгом 7000 PLN!')) {
        ...
        const newState = {
            balance: 0, debt: 7000, debtTimer: Date.now(), debtOverdue: true,
            isOnline: false, isSearching: false, scheduledOffline: false,
            items: { bike: 100, bag: 100, phone: 100, gear: 100 },
            needs: { energy: 100, water: 100, mood: 100 },
            inventory: { water: 0, bar: 0, energy_drink: 0, coffee: 0 },
